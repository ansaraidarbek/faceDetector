...
def split_folds(X_train, y_train, num_folds):
    ...
    #       START YOUR CODE HERE        #
    #####################################   
    xt_splitted = np.array(np.array_split(X_train, num_folds))
    yt_splitted = np.array(np.array_split(y_train, num_folds))
    for i in range(num_folds):
        X_trains[i] = xt_splitted[(np.arange(num_folds)!=i)].reshape((-1, X_trains.shape[-1]))
        X_vals[i] = xt_splitted[i]
        y_trains[i] = yt_splitted[(np.arange(num_folds)!=i)].reshape((-1, y_trains.shape[-1]))
        y_vals[i] = yt_splitted[i]
    ######################################
    #        END OF YOUR CODE            #
    ######################################
